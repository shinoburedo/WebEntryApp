 ...
 * @author r-ehara
 */
public class DntInpConfAct extends AbstractAction {
    ...
    protected String doProcess(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (request.getParameter("submit") != null) {
            ...
            if (state) {
                // 団体テーブル・申込団体にINSERTする
                dntServ.registDantai(dntInfo);
                // 団体申込出願完了メールを送信する
                if (mailSendServ.sendDntResuMail(dntInfo)) {
                    //メール送信成功
                    // リクエストに情報を保存
                    ...
                    // メール送信失敗
                    LogGenerate.errorOutput("団体コード："
                            + dntInfo.getDantaiCode() + "の団体申込出願完了メール送信が失敗しました。エラー画面に遷移します。");
                    throw new Exception("団体コード：" + dntInfo.getDantaiCode() + "団体申込者登録用ID：" + dntInfo.getDantaiMoshikomiUketsukeNo() + "メール送信失敗");
                }
            } else {
    ...
    private boolean ValidationCaller(HttpServletRequest request, DntInf dntJoho) throws Exception {
        ...
        errors = dntServ.publicValidationCaller(request, dntJoho, true, true);
