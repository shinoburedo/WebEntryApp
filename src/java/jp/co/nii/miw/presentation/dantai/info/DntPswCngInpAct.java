 ...
 * @author r-ehara
 */
public class DntPswCngInpAct extends AbstractAction {
    ...
    protected String doProcess(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (request.getParameter("submit") != null) {
            ...
            if (state) {
                ...
                //団体テーブルを検索し、現在のパスワードが正しいか確認
                Dantai dantai = new Dantai(MiwConstants.DS_REGISTRANT).find(dntInf.getDantaiCode());
                if (password.equals(dantai.getDantaiPasswd())) {
                    //パスワード更新
                    dntInfServ.updatePassword(dantai,passwordNew1,dntInf.getDantaiCode());
                    //完了画面へ遷移
                    ret = FWD_NM_SUCCESS;
                } else {
                    //現在のパスワードエラー
