...
/**
 * タイトル:DntPswCngInpInitAct
 * 説明:DntPswCngInpInitActの初期化アクションクラス
 * 著作権:   Copyright (c) 2012
 * 会社名:   日本情報産業株式会社
 * @author --r.ehara
 */
public class DntPswCngInpInitAct extends AbstractAction {
    ...
    protected String doProcess(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (MiwConstants.FLG_OFF.equals(dntInfoMenu.getDntPswUpdKigenFlg())){
            // 期間外のためセッションを切る
            session.invalidate();
            ret = FWD_NM_SESSION;
        }else{
