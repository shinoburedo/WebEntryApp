 ...
 *
 * 著作権: Copyright (c) 2010 会社名: 日本情報産業株式会社
 *
 * @author --t.yamaguchi
 */
public class MskPicSelAct extends AbstractAction {
    ...
    protected String doProcess(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (request.getParameter("submit") != null) {//「次　へ」ボタンが押下されたら
            ...
            //MskShikeから画像ＩＤを取得
            String gazoId = mskTorokuJoho.getGazoId();

            // 画面遷移に影響するサービスはexecuteメソッド内で呼び出す（インターフェース経由）
            // 入力パラメータのチェックの呼び出し
            String retCheck = ValidationCaller(request, form, fileUp, gazoId);
...
//                mskTorokuJoho.setGazoData(gazoFile);
     ...
     * @throws Exception
     */
    private String ValidationCaller(HttpServletRequest request, ActionForm form, FormFile fileUp, String GazoID) throws Exception {
