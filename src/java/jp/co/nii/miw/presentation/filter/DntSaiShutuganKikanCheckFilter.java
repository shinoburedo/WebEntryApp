 ...
 * @author r.ehara
 */
public class DntSaiShutuganKikanCheckFilter implements Filter {
    ...
    public final void doFilter(final ServletRequest req,
        ...
        try {
            //保持した情報を取得する
            MenuDntInf dntMenu = (MenuDntInf) session.getAttribute("MenuJoho");

            if (MiwConstants.FLG_OFF.equals(dntMenu.getDntMskKigenFlg())) {
                //団体再出願期間ではない
                LogGenerate.errorOutput(
                        this.getClass().getName() + "　団体再出願期間内ではありません", "none");
