...
/**
 * <p>タイトル: フィルタクラス</p> <p>説明: 期間以内かのチェックを行う</p> <p>著作権: Copyright (c) 2010</p>
 * <p>会社名: 日本情報産業株式会社</p>
 *
 * @author T.Yamaguchi
 */
public class JznKikanCheckFilter implements Filter {
    ...
    public final void doFilter(final ServletRequest req,
        ...
        if (!isExcludePath(thisURL)) {
            ...
            try {
                ...
                if (!jznTorokuJoho.getIsIDKikan()) {
                    //ＩＤ取得期間ではない
                    LogGenerate.errorOutput(
                            this.getClass().getName() + "　ＩＤ取得期間内ではありません", "none");
